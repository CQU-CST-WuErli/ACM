...
int main(int argc, char const *argv[]) {
    ...
    while(t--) {
        ...
        while(1) {
            ...
            for (int i=1;i<=n;i++) if (!vis[i] && i!=tmp) {
            	if (Cross(p[tmp]-p[id],p[i]-p[id])<0) {
            		tmp=i;
            	}
            	else if (Cross(p[tmp]-p[id],p[i]-p[id])==0) {
            		if (DistanceToPoint(p[id],p[tmp])>DistanceToPoint(p[id],p[i])) {
            			tmp=i;
            		}
            	}
            }
