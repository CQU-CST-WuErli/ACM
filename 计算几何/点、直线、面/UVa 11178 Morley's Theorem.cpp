...
int main(int argc, char const *argv[]) {
    ...
    while(n--) {
        ...
        vector<Point> ans;
        Vector AB=B-A,BA=A-B;
        Vector BC=C-B,CB=B-C;
        Vector AC=C-A,CA=A-C;
        double ABC=Angle(BC,BA),BAC=Angle(AB,AC),ACB=Angle(CA,CB);
        Vector AF=Rotate(AB,BAC/3);
        Vector AE=Rotate(AC,2*pi-BAC/3);
        Vector BF=Rotate(BA,2*pi-ABC/3);
        Vector BD=Rotate(BC,ABC/3);
        Vector CE=Rotate(CA,ACB/3);
        Vector CD=Rotate(CB,2*pi-ACB/3);
        ans.push_back(GetLineIntersection(B,BD,C,CD));
        ans.push_back(GetLineIntersection(A,AE,C,CE));
        ans.push_back(GetLineIntersection(A,AF,B,BF));
        printf("%.6f %.6f %.6f %.6f %.6f %.6f\n",ans[0].x,ans[0].y,ans[1].x,ans[1].y,ans[2].x,ans[2].y);
    }
	return 0;
}