...
#endif
    while(SI(n)==1 && n) {
        rep(i,1,n) cin >> sq[i].len,sq[i].left=sq[i].right=0;
        sq[1].left=0;
        rep(i,1,n) {
        	rep(j,1,i-1) {
        		if (sq[i].len>sq[j].len) {
        			sq[i].left=max(sq[i].left,sq[j].right+sq[j].len/sqrt(2)-sq[i].len/sqrt(2));
        		}
        		else if (sq[i].len==sq[j].len) {
        			sq[i].left=max(sq[i].left,sq[j].right);
        		}
        		else {
        			sq[i].left=max(sq[i].left,sq[j].right-sq[j].len/sqrt(2)+sq[i].len/sqrt(2));
        		}
        	}
        	sq[i].right=sq[i].left+sq[i].len*sqrt(2);
        }
        // rep(i,1,n) cout << sq[i].left << ' ' << sq[i].right << endl;
        rep(i,2,n) {
        	rep(j,1,i-1) if (sq[i].left<sq[i].right) {
        		if (sq[i].len>sq[j].len && sq[j].right>sq[i].left) {
        			sq[j].right=sq[i].left;
        		}
        		else if (sq[i].len<sq[j].len && sq[j].right>sq[i].left) {
        			sq[i].left=sq[j].right;
        		}
        	}
        }
        rep(i,1,n) if (sq[i].right-sq[i].left>eps) cout << i << ' ';
        cout << endl;
    }
	return 0;
}