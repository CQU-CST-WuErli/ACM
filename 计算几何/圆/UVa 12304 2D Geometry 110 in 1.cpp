...
#endif
    while(cin >> s) {
    	double X1,Y1,X2,Y2,X3,Y3,R1,R2,X4,Y4;
        if (s=="CircumscribedCircle") {
        	scanf("%lf%lf%lf%lf%lf%lf",&X1,&Y1,&X2,&Y2,&X3,&Y3);
        	Circle c=circumscribedCircle(Point(X1,Y1),Point(X2,Y2),Point(X3,Y3));
        	printf("(%.6f,%.6f,%.6f)\n", c.c.x,c.c.y,c.r);
        }
        else if (s=="InscribedCircle") {
        	scanf("%lf%lf%lf%lf%lf%lf",&X1,&Y1,&X2,&Y2,&X3,&Y3);
        	Circle c=inscribedCircle(Point(X1,Y1),Point(X2,Y2),Point(X3,Y3));
        	printf("(%.6f,%.6f,%.6f)\n", c.c.x,c.c.y,c.r);
        }
        else if (s=="TangentLineThroughPoint") {
        	scanf("%lf%lf%lf%lf%lf",&X1,&Y1,&R1,&X2,&Y2);
        	Vector v[2];
        	int num=getTangents(Point(X2,Y2),Circle(Point(X1,Y1),R1),v);
        	printf("[");
        	vector<double> ans;
        	for (int i=0;i<num;i++) {
        		double tmp=angle(v[i]);
        		tmp=tmp/pi*180;
        		while(dcmp(tmp)<0) tmp+=360.0;
 				while(dcmp(tmp-180) >= 0) tmp-= 180.0;
        		ans.push_back(tmp);
        	}
        	...
        	scanf("%lf%lf%lf%lf%lf%lf%lf",&X1,&Y1,&X2,&Y2,&X3,&Y3,&R1);
        	vector<Point> v;
        	Circle c=Circle(Point(X1,Y1),R1);
        	Line L=getLine(X2,Y2,X3,Y3);
        	L=L.move(R1);
        	int n1=getLineCircleIntersection(L,c,v);
        	L=L.move(-2*R1);
        	int n2=getLineCircleIntersection(L,c,v);
        	// cout << n1 << ' ' << n2 << endl;
        	sort(v.begin(),v.end());
        	printf("[");
        	for (int i=0;i<v.size();i++) {
        		if (i) printf(",");
        		printf("(%.6f,%.6f)",v[i].x,v[i].y);
        	}
        	printf("]\n");
        }
        else if (s=="CircleTangentToTwoLinesWithRadius") {
        	scanf("%lf%lf%lf%lf%lf%lf%lf%lf%lf",&X1,&Y1,&X2,&Y2,&X3,&Y3,&X4,&Y4,&R1);
        	Line a=getLine(X1,Y1,X2,Y2);
        	Line b=getLine(X3,Y3,X4,Y4);
        	vector<Point> ans;
        	Line L1 = a.move(-R1), L2 = a.move(R1);
		    	Line L3 = b.move(-R1), L4 = b.move(R1);
		    	ans.push_back(GetLineIntersection(L1.p,L1.v,L3.p,L3.v));
  		  	ans.push_back(GetLineIntersection(L1.p,L1.v,L4.p,L4.v));
  		  	ans.push_back(GetLineIntersection(L2.p,L2.v,L3.p,L3.v));
  		  	ans.push_back(GetLineIntersection(L2.p,L2.v,L4.p,L4.v));
  		  	sort(ans.begin(),ans.end());
  		  	printf("[");
  		  	for (int i=0;i<ans.size();i++) {
  		  		if (i) printf(",");
        	...
        	Circle c1=Circle(Point(X1,Y1),R1+r);
        	Circle c2=Circle(Point(X2,Y2),R2+r);
        	printf("[");
        	double dist=Length(c2.c-c1.c);
