...
void hash_table() {
	...
	for (int i=1;i<=n;i++) {
		l[i]=lower_bound(ALL(Hash),l[i])-Hash.begin()+1;
		r[i]=lower_bound(ALL(Hash),r[i])-Hash.begin()+1;
	}
}

int a[500],vis[500],p[500];
ll d[500];
bool spfa(int s,int t,int &f,ll &Cost){
	CLR(a);CLR(vis);
...
//	freopen("C:\\Users\\john\\Desktop\\out.txt","w",stdout);
	...
	while (kase--){
		...
		add_edge(s,1,k,0);
		add_edge(1,s,0,0);
		add_edge(Hash.size(),t,k,0);
		add_edge(t,Hash.size(),0,0);
