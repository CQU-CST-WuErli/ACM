...
const int N=1e5;
int pnt[N],nxt[N],pre[N],head[N],cap[N],cost[N];
int cnt;
int n,m;
int row[30],col[30];
int add_edge(int u,int v,int c,int w){
	pnt[cnt]=v;nxt[cnt]=head[u];pre[cnt]=u;head[u]=cnt;
	cap[cnt]=c;cost[cnt++]=w;
}	
	
	
int level[100],iter[100];
...
//	freopen("C:\\Users\\john\\Desktop\\out.txt","w",stdout);
	...
	while (T--) {
		...
		for (int j=1;j<=m;j++) scanf("%d",col+j);
		for (int i=n;i>=1;i--) row[i]=(i==1?row[i]:row[i]-row[i-1]);
//		for (int i=1;i<=n;i++) lookln(row[i]);
		for (int j=m;j>=1;j--) col[j]=(j==1?col[j]:col[j]-col[j-1]);
//		for (int j=1;j<=m;j++) lookln(col[j]);
		for (int i=1;i<=n;i++){
			add_edge(s,i,row[i]-m,0);
			add_edge(i,s,0,0);
		}
		for (int j=1;j<=m;j++){
			add_edge(j+n,t,col[j]-n,0);
			add_edge(t,j+n,0,0);
		}
		for (int i=1;i<=n;i++){
			for (int j=m;j>=1;j--){
				add_edge(i,j+n,19,0);
				add_edge(j+n,i,0,0);            // just for print the ans
			}
		}
		Dinic(s,t);
		printf("Matrix %d\n",kase++);
