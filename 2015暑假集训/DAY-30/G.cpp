...
//	freopen("C:\\Users\\john\\Desktop\\out.txt","w",stdout);
	...
	while (T--){
		char ss[30],tt[30];
		For_UVa;
		cnt=0;OFF(head);ID_start=1;
		
		SI(n);
		for (int i=1;i<=n;i++){
			scanf("%s",tt);
			target[i]=get_id(tt);
//			add_edge(target[i],t,1,0);
//			add_edge(t,target[i],0,0);
		}
		SI(m);
		for (int i=1;i<=m;i++){
			scanf("%s",tt);
			scanf("%s",tt);
			device[i]=get_id(tt);
//			add_edge(s,device[i],1,0);
//			add_edge(device[i],s,0,0);
		...
		for (int i=1;i<=k;i++){
			...
			scanf("%s",ss);
			to[i]=get_id(ss);
//			add_edge(from[i],to[i],INF,0);
//			add_edge(to[i],from[i],0,0);
		}
		int s=0,t=mp.size()+2;
		for (int i=1;i<=n;i++) {
			add_edge(target[i],t,1,0);
