...
int pnt[N],nxt[N],head[N],cap[N],cost[N];
int g[300][300];
int ans[300][300];
int X[300],Y[300],P[300],Q[300],B[300],C[300];
int n,m;
int cnt;
...
int vis[300],num[300];int d[300];
int pre[300];
int spfa(int t,int n){
	CLR(vis);CLR(num);
	queue<int> q;
	q.push(t);
	vis[t]=1;num[t]=1;
	...
	while (SII(n,m)==2){
		...
		for (int i=1;i<=n;i++){
			for (int j=1;j<=m;j++){
				int x; SI(x);
				ans[i][j]=x;
				if (x!=0) add_edge(j+n,i,0,-g[i][j]);
				sum[j]+=x;
			}
		}
		for (int i=1;i<=m;i++) {
			if(sum[i]<C[i]) add_edge(i+n,T,0,0);
			if (sum[i]>0) add_edge(T,i+n,0,0); 
		}
		int start=spfa(T,T);
		if (start==-1) {
			cout << "OPTIMAL\n";
			...
			CLR(used);
			int tmp;
			for (tmp=start;!used[tmp];tmp=pre[tmp]){
				used[tmp]=1;
			}
			start=tmp;
			// tmp -> the start point of the minus circle
//			lookln(tmp);
			do {
				int u=pre[tmp],v=tmp;
				if (u<=n && v>n && v<T) ans[u][v-n]++;
				if (v<=n && u>n && u<T) ans[v][u-n]--;
				tmp=pre[tmp];
			}while (tmp!=start);
	 		for (int i=1;i<=n;i++) {
 				for (int j=1;j<=m;j++){
