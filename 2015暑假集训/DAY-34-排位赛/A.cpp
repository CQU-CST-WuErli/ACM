...
int n,m;
int t[200];
string bug[200],state[200]; 

string trans(int sta){
	string tt="";
	while (sta){
		if (sta%2) tt+="1";
		else tt+="0";
...
bool check(int sta,int id){
	for (int i=n-1;i>=0;i--){
		if (bug[id][i]=='+' && (sta&1)==0) return false;
		if (bug[id][i]=='-' && (sta&1)==1) return false;
		sta>>=1;
	}	
	return true;
}

int fix(int sta,int id){
	string tt=trans(sta);
//	string tmp="";
	int ret=0;
	for (int i=0;i<n;i++) {
		if (state[id][i]=='-') ret=ret<<1;
		else if (state[id][i]=='+') ret=(ret<<1)|1;
		else ret=(ret<<1)|((sta>>(n-i-1))&1);
	}
	return ret;
}
