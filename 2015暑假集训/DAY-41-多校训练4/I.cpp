...
//	lookln(cnt);
	for (int i=1;i<20001;i++){
		...
		int p=prime[i];
		if (p>=1 && p<10) tmp=1;
		else if (p>=10 && p<100) tmp=2;
		else if (p>=100 && p<1000) tmp=3;
		else if (p>=1000 && p<10000) tmp=4;
		else if (p>=10000 && p<100000) tmp=5;
		else if (p>=100000 && p<1000000) tmp=6;
		len[i]=len[i-1]+tmp;
		string tt;
//		lookln(p);
		while (p){
...
//	lookln(cnt);
	while (cin >> n){
		int l=len[n];
//		for (int i=1;i<=len[n];i++) cout << s[i];
//		cout << endl;
		int sum=1;
		int ans=0;
		for (int i=1;;i+=2){
