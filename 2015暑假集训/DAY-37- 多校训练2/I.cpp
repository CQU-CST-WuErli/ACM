...
void Dijkstra(){
	...
	while (!q.empty()){
		...
		vis[x.second]=1;
		for (int i=0;i<pro[x.second].size();i++){
			int v=pro[x.second][i];
			in[v]--;
			p[v]=max(p[v],d[x.second]);
			if (d[v]!=INF && in[v]==0){
				d[v]=max(d[v],p[v]);
				q.push(P(d[v],v));
			}
		...
		for (int i=head[x.second];~i;i=nxt[i]){
			int v=pnt[i];
			if (d[v]>d[x.second]+w[i]){
				d[v]=max(d[x.second]+w[i],p[v]);
				if (in[v]==0) q.push(P(d[v],v));
			}
		}
	}
}
