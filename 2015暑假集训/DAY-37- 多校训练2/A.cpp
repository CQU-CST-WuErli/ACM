...
ll __lcm(ll a,ll b){
	return a/__gcd(a,b)*b %100000007;
}
...
int dfs(ll u,ll x){
	if (k%x) return 0;
	if (u==n && x==k) return dp[u]=1;
	if (dp[u]!=-1) return dp[u];
	dp[u]=0;
	for (ll i=head[u];~i;i=nxt[i]){
		ll v=pnt[i];
		ll tmp=__lcm(x,w[v]);
		if (k%tmp || tmp==x) continue;
		dp[u]=(dp[u]+dfs(v,tmp))%(1000000007);
	}	
	return dp[u]%(1000000007);
}
...
//	freopen("C:\\Users\\john\\Desktop\\out.txt","w",stdout);
	...
	while (SIII(n,m,k)==3){
		...
		OFF(dp);	
		cout << dfs(1,w[1])%1000000007 << endl;
	}
	return 0;
}
