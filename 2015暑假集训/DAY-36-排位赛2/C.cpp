...
#endif
	while (SI(n)==1) {
		for (int i=1;i<=n;i++) cin >> pos[i] >> t[i];
		CLR(dp);
		for (int i=1;i<=n;i++){
			for (int j=i-1;j>=1;j--){
				dp[j][i][0]=INF;
				dp[j][i][0]=min(dp[j+1][i][1]+pos[i]-pos[j],dp[j+1][i][0]+pos[j+1]-pos[j]);
				if (dp[j][i][0]>=t[j]) dp[j][i][0]=INF;
				dp[j][i][1]=INF;
				dp[j][i][1]=min(dp[j][i-1][0]+pos[i]-pos[j],dp[j][i-1][1]+pos[i]-pos[i-1]);
				if (dp[j][i][1]>=t[i]) dp[j][i][1]=INF;
			}
		}
