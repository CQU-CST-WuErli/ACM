...
//	freopen("C:\\Users\\john\\Desktop\\out.txt","w",stdout);
	...
	while (cin >> n >> d){
		...
		cnt=v.size()-1;
		for (int i=1;i<=n;i++){
			int l=lower_bound(ALL(v),a[i]-d)-v.begin()-1;
			int r=upper_bound(ALL(v),a[i]+d)-v.begin()-1;
			ll tmp=query(r)-query(l);
			int ID=lower_bound(ALL(v),a[i])-v.begin();
			update(ID,tmp+1);
		}
		cout << ((query(cnt)-n)%MOD+MOD) %MOD << endl; 
	}
	return 0;
}
