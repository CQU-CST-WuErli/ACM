...
const string tt="LOVE\0";

const int N=2e4+10;
int pnt[N],nxt[N],head[N],cost[N],pre[N],id[N];
int cnt;
int n,m;

void add_edge(int u,int v,int w,char c){
	pnt[cnt]=v;nxt[cnt]=head[u];id[cnt]=c;cost[cnt]=w;
	head[u]=cnt++;
}
...
void spfa(){
	...
	while (!q.empty()){
		...
		int k=(x.second+1)%4;
		for (int i=head[x.first];i!=-1;i=nxt[i]){
			int v=pnt[i];
			if (d[v][k]>d[x.first][x.second]+cost[i]){
				d[v][k]=d[x.first][x.second]+cost[i];
				cur[v][k]=cur[x.first][x.second]=1;
				if (!vis[v][k]){
					vis[v][k]=1;
					q.push(P(v,k));
				} 
			}
			else if (d[v][k]==d[x.first][x.second]+cost[i]){
				cur[v][k]=max(cur[v][k],cur[x.first][x.second]+1);
				if (!vis[v][k]){
					vis[v][k]=1;
					q.push(P(v,k));
				} 
	...
	int ans=0;
	if (d[n][3]>=0x7fffffff || cur[n][3]<4) {
		printf("Binbin you disappoint Sangsang again, damn it!\n");
	}
	else {
		printf("Cute Sangsang, Binbin will come with a donkey after travelling %lld meters and finding %d LOVE strings at last.",d[n][3],cur[n][3]/4);
	}
}
