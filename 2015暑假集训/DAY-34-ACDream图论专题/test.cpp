/*
* this code is made by WEL9624579
* Problem: 1744
* Verdict: Wrong Answer
* Submission Date: 2015-08-21 14:09:11
* Time: 4MS
* Memory: 1744KB
*/
//
...
bool dfs(int u,int step){
    ...
    for (int i=0;i<n;i++) if (g[u][i]){
        ...
        else if (!c[i] && !dfs(i,step+1)) return false;
    } 
    c[u]=1;topo[cnt--]=u;
    return true;
}
...
//  freopen("C:\\Users\\john\\Desktop\\out.txt","w",stdout);
    ...
    while (SIII(n,m,k)==3){
        ...
        int flag=0;
        for (int i=0;i<n;i++) if (toposort(i)){
            flag=1;break;
        }
        if (flag){
            for (int i=1;i<=k;i++) {
