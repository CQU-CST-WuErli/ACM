...
int n, m, q[N];
double sum[N], rev[N], pre[N], f[55][N];

//f[i] = { f[j] + pre[i]-pre[j]-sum[j]*(rev[i]-rev[j]) };
//slope(k, j) = { (f[j]-pre[j]+sum[j]*rev[j]) - () } / {sum[j] - sum[k]} < rev[i];

double up(int p, int k, int j) {
    return (f[p][j] - pre[j] + sum[j] * rev[j]) -
           (f[p][k] - pre[k] + sum[k] * rev[k]);
}
