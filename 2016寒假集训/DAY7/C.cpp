...
const int MAX_L=3005;// For BigInteger
...
int main(){
#ifdef LOCAL
//	freopen("C:\\Users\\john\\De8sktop\\in.txt","r",stdin);
//	freopen("C:\\Users\\john\\Desktop\\out.txt","w",stdout);
	...
	while (SI(n)==1) {
		...
		CLR(vis);
		Rep(i,2000,0) rep(j,1,n) if (op[j]==0 && xx[j]==i) {
			int k;
			for (k=j;k>=1;k--) if (vis[k] || (op[k] && xx[k]==i)) break;
			if (k==0 || vis[k]) continue;
			for (int s=k;s<=j;s++) vis[s]=1;
			ans+=ksm(i);
		}	
		cout << ans << endl; 
	}
